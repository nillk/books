...
`Either`의 추상 개념은 원하는 개수만큼 슬롯을 확장할 수 있다. 일례로 `Either<Empty, Either<Leaf, Node>>` 같은 선언을 생각해보자. 이 자료구조의 각 *셀*은 가능한 세 자료형 중에서 한 번에 한 개만 가질 수 있는, 전통적인 컴퓨터과학에서의 `union`에 해당한다.
